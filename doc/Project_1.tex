...
\section*{\large Analytic expressions}
    ...
    \begin{equation}
        ...
        \end{array}
    \end{equation}
    where $\omega_{ho}$ defines the trap potential, and
    \begin{equation}
            H
        =   \sum_i^N \left(\frac{-\hbar^2}{2m} \nabla_i^2 + V_\txt{ext}\right) + \sum_{i<j}^N V_\txt{int}
    \end{equation}
    is the two-body Hamiltonian.
    The inter-boson interaction is represented by a pairwise repuslive potential
    \begin{equation}
    ...
    \begin{equation}
        ...
        =   e^{-\alpha (x^2 + y^2 + z^2)}
    \end{equation}
    such that the local energy in one dimension
    \begin{equation}
            E_L 
